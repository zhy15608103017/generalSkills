...
本文档说明 `code-review-loop` 的模型、`.env`、双模型和命令行参数配置方式。这些配置是通用的，不绑定具体项目。
...
shell 环境变量会覆盖 `.env`。`.env` 只会补充当前进程中尚未设置的变量。
...
```text
always: PRIMARY 和 SECOND 都运行
auto:    PRIMARY 先审，达到触发条件后再运行 SECOND
off:     只运行 PRIMARY
当 PRIMARY 和 SECOND 都成功时，最终结果会合并
任意一个模型发现 P0/P1 都会形成阻塞
任意一个模型调用失败或超时时，会降级使用已成功模型的结果，并在 verification_notes 和报告中记录原因
```
...
`AI_REVIEW_SECOND_API_KEY` 只提供凭证，单独设置它并不会启用第二轮审查。
...
```text
always  第二模型配置存在且凭证可用时，强制运行第二轮审查。
auto    默认值。只有当主模型返回的 P0/P1/P2 达到阈值时，才运行第二轮审查。
off     完全关闭第二轮审查，即使配置了第二模型也不会运行。
```
...
在 `auto` 模式下，主模型返回的 P0/P1/P2 数量达到阈值，或主审 `confidence` 小于 `AI_REVIEW_SECOND_CONFIDENCE_THRESHOLD` 时，都会触发第二审。

第二模型默认继承主审当前已生效的请求预算，包括 `high-accuracy` profile、`--timeout-ms`、`--retries`、`--retry-fast-failure-ms`、`--retry-delay-ms` 以及对应环境变量带来的覆盖。

也就是说：

- 如果主审因为 `high-accuracy` 使用了更长超时，第二审默认也会继承这组预算。
- 如果显式设置了 `--second-timeout-ms` / `AI_REVIEW_SECOND_TIMEOUT_MS`、`--second-retries` / `AI_REVIEW_SECOND_RETRIES`、`--second-retry-fast-failure-ms` / `AI_REVIEW_SECOND_RETRY_FAST_FAILURE_MS` 或 `--second-retry-delay-ms` / `AI_REVIEW_SECOND_RETRY_DELAY_MS`，则第二审使用自己的独立值。
...
AI_REVIEW_SECOND_LOCAL_CLI     第二模型本地 AI CLI 预设，可选 claude、opencode、codex
...
AI_REVIEW_SECOND_CONFIDENCE_THRESHOLD auto 模式下主模型低置信度触发阈值，默认 0.8
...
AI_REVIEW_MAX_REVIEW_ROUNDS  审查/修复闭环最大轮数，默认 3；设为 infinity 表示无上限
...
AI_REVIEW_CLI_COMMAND        主模型 CLI 审查命令
AI_REVIEW_MAX_SHARDS         自动分片最大并行数，默认 4，最高 8；只限制上限，不决定是否分片
...
`model-providers.json` 只描述 provider 默认值和服务商自身环境变量，例如：
...
不要在这里写 `AI_REVIEW_PRIMARY_*` 或 `AI_REVIEW_SECOND_*`。这些变量属于运行时路由配置，应由脚本单独解析，避免第一模型、第二模型和 provider 默认值混在一起。
...
调用本机已安装的 `claude`、`opencode` 或 `codex`，无需为每个 CLI 单独写包装脚本：
...
```text
claude    执行 claude -p --output-format text，审查 brief 通过 stdin 传入
codex     执行 codex exec --color never --ephemeral <instruction>，审查 brief 通过 stdin 传入
opencode  将审查 brief 写入临时文件，执行 opencode run --file <prompt-file> <instruction>
```
...
如果某个 CLI 版本的非交互参数与内置 preset 不一致，可改用 `AI_REVIEW_CLI_COMMAND` / `AI_REVIEW_SECOND_CLI_COMMAND` 兜底。
...
`--profile auto` 是标准入口。大型改动会自动拆成并行分片，并在分片完成后运行汇总审查；小型和中型改动仍自动使用单次代码审查。`--max-shards` 只限制自动分片的最大并行数，不要求用户手动选择是否分片。

覆盖第二审的独立超时、重试和 auto 置信度阈值：
...
`--codegraph` 会运行 `codegraph status -j <repo>` 和 `codegraph affected -p <repo> -d <depth> -j -- <changed-files>`，并把输出写入审查 brief 的 `CodeGraph 影响分析` 段落。CodeGraph 未安装、未初始化或命令失败时，脚本会记录失败原因并继续审查。
...
`--profile auto` 会根据改动规模和风险自动选择单次审查或并行分片加汇总审查。
...
- 不要把 `.ai-review/latest-brief.md` 上传到公开位置，它可能包含本地代码上下文。
- 内置脱敏规则只覆盖常见环境变量、HTTP bearer token、JSON key 字段和 CLI 参数形态；它用于降低误传风险，不应被视为完整 DLP 或安全边界。
- CLI command 只能来自可信配置。
- 审查上下文过大时，优先用 `--path` 缩小范围，而不是盲目增大限制。
- 高风险改动建议启用双模型，并继续使用 `--profile auto` 让脚本自动升级审查策略。

## `.ai-reviewignore`

可在仓库根目录创建 `.ai-reviewignore`，排除部分文件不参与 review。
...
如果只是临时缩小本次 review 范围，而不是维护长期排除规则，仍然优先使用 `--path` 或 `--paths`。
...
或确认 provider 在 `model-providers.json` 中存在默认 `baseUrl`。

...
description: 当功能、修复、重构或其他代码改动在完成前需要额外的本地 AI 代码审查时使用；先预检显式 reviewer 配置，未配置时报告审核未运行并按宿主能力降级。纯文档、纯格式、纯注释、纯错字、锁文件、生成产物、`.ai-review/` 工件或仅依赖版本变更不触发。
---
...
- 正式审核前先运行 `--check-config`。只有显式 API reviewer 路由及其必要凭证，或显式本地 AI CLI reviewer 配置可解析时，才进入需求理解审计和代码审核。
- 每次正式审查前，都必须创建或更新 `.ai-review/review-context/current-request.md`；单独运行 `--check-config` 或 `--dry-run` 不要求该文件。
...
- 预检通过：继续准备上下文并运行正式 AI 审核。
- 预检失败：不要进入需求理解审计、代码审核或审核轮次计数。优先使用宿主中已显式授权且能够独立返回可核验结论的审核能力；若不存在，则执行确定性本地验证和当前 agent 自查。最终必须报告“独立 AI 审核未运行/审核配置未就绪”，不能把降级检查表述为 `code-review-loop pass`。
- 本工具支持显式配置的 API reviewer、`local-cli`、`claude`、`codex`、`opencode` 或可信自定义 CLI。不要根据已安装命令、provider 专属 API key 或其他环境线索静默选择 reviewer。
...
1. 运行 `--check-config` 预检主审配置。失败时停止正式 AI 闭环，优先使用宿主中已显式授权的独立审核能力；没有该能力时再执行本地验证与当前 agent 自查，并报告审核配置未就绪。
2. 预检通过后，创建或更新 `.ai-review/review-context/current-request.md`，写入原始请求、用户纠正、当前理解、反例、设计、非目标、验收标准和建议验证命令。
3. 用 `git status --short` 确认本地确实存在待审查改动。
4. 先运行与改动匹配的本地验证命令，例如 `git diff --check`、`npm test`。修改本 skill 时可运行 `node .agents/skills/code-review-loop/scripts/check-syntax.mjs` 检查全部 `.mjs`。
5. 如有需要，可先跑一次 `--dry-run`，只检查审查简报内容而不调用模型。`dry-run` 可以在请求上下文文件尚未存在时运行；正式审核仍然要求 `.ai-review/review-context/current-request.md` 已准备好。
6. 在仓库根目录运行内置审查脚本。它会再次快速检查配置，然后先跑需求理解闸门，通过后再进入代码审查：

```bash
node .agents/skills/code-review-loop/scripts/ai-review.mjs --profile auto --verify "git diff --check"
```

7. 运行期间，如需查看进度，可检查 `.ai-review/latest-status.md` 或终端 heartbeat 输出。
8. 读取 `.ai-review/latest-result.json` 和 `.ai-review/latest-report.md`；如果是需求闸门拦截，也要查看需求审计相关产物。
9. 汇报全部 findings，但在结束前必须修复所有阻塞性的 `P0/P1` 问题。
10. 阻塞问题需要由当前编码 agent 自己修复，然后重新跑本地验证。
11. 按配置上限重复“审查/修复”闭环。工具会持久化当前请求的连续未通过轮次，默认最多 `3` 轮；`infinity` 表示不设上限。
12. 如果达到最大轮次后仍有阻塞问题，停止自动循环，并把剩余问题交还给人工决策。
...
配置预检失败也使用 `needs_human`，但它表示审核能力未就绪，不表示需求审计或代码审查已经运行。
